report 50037 "PurchaseGRNExt"
{
    ...
    RDLCLayout = './src/reportextension/PurchaseDocumentTestExt.rdlc';
...
then
                        ...
                        if Item.Get(PurchaseLine."No.") then begin
                            if Item.Blocked then
                                AddError(
                                  StrSubstNo(
                                    MustBeForErr,
                                    Item.FieldCaption(Blocked), false, Item.TableCaption, PurchaseLine."No."));
                                ...
                                if Item.Reserve = Item.Reserve::Always then begin
                                    ...
                                    if (PurchaseLine.Signed(PurchaseLine.Quantity) < 0) and (Abs(PurchaseLine."Reserved Quantity") < Abs(PurchaseLine."Qty. to Receive")) then
                                        AddError(
                                          StrSubstNo(
                                            Text019,
                                            PurchaseLine.FieldCaption("Reserved Quantity"), PurchaseLine.Signed(PurchaseLine."Qty. to Receive")));
                                end;
                        end else
                            AddError(
                              StrSubstNo(
                                Text008,
                                Item.TableCaption, PurchaseLine."No."));
                end;
    ...
    begin
        ...
        TableID[1] := DimMgt.TypeToTableID3(PurchLine.Type.AsInteger());
        No[1] := PurchLine."No.";
        TableID[2] := DATABASE::Job;
