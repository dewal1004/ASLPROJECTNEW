report 50220 "Apapa Fish Shop Sales Report"
{
    //   "Sea Food categories"
    DefaultLayout = RDLC;
    ...
    Caption = 'Apapa Fish Shop Sales Report';
    dataset
    {
        dataitem("Sea Food categories"; "Sea Food categories")
        {
            ...
            column(Apapa_Fish_Shop_Sales_ReportCaption; Apapa_Fish_Shop_Sales_ReportCaptionLbl)
            {
            }
            ...
            dataitem(Item; Item)
            {
                DataItemTableView = SORTING("SF Cat", "No. 2") WHERE("Gen. Prod. Posting Group" = FILTER('FIS'), "Sales (Qty.)" = FILTER(<> 0), "Location Filter" = FILTER('APFISHSHOP'));
    ...
    var
        ...
        Apapa_Fish_Shop_Sales_ReportCaptionLbl: Label 'Apapa Fish Shop Sales Report';
