report 90003 "Landed Costxx"
{
    ...
    begin
        ...
        */
        /*//Actual Cost retrieval from the GL
        GenLed.SETCURRENTKEY("External Document No.","G/L Account No.","Reason Code");
        GenLed.SETRANGE(GenLed."External Document No.","Purchase Header"."No.");
        GenLed.SETRANGE(GenLed."G/L Account No.",'2187');
          GenLed.SETRANGE(GenLed."Reason Code",'FREIGHT');
           IF GenLed.FIND('-') THEN F:=GenLed.Amount ELSE F:=0;
             K[7]:=F;
          GenLed.SETRANGE(GenLed."Reason Code",'ANCILLIARY');
           IF GenLed.FIND('-') THEN G:=GenLed.Amount ELSE G:=0;
             K[8]:=G;
          GenLed.SETRANGE(GenLed."Reason Code",'CLEARING');
           IF GenLed.FIND('-') THEN H:=GenLed.Amount ELSE H:=0;
             K[9]:=H;
          ...
          GenLed.SETRANGE(GenLed."Reason Code",'DUTY');
            IF GenLed.FIND('-') THEN K[2]:=GenLed.Amount ELSE K[2]:=0;
          GenLed.SETRANGE(GenLed."Reason Code",'ECOWAS');
            IF GenLed.FIND('-') THEN K[3]:=GenLed.Amount ELSE K[3]:=0;
          GenLed.SETRANGE(GenLed."Reason Code",'INSURANCE');
            IF GenLed.FIND('-') THEN K[4]:=GenLed.Amount ELSE K[4]:=0;
          GenLed.SETRANGE(GenLed."Reason Code",'SURCHARGE');
            IF GenLed.FIND('-') THEN K[5]:=GenLed.Amount ELSE K[5]:=0;
